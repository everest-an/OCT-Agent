...
**Claw CLI ↔ Desktop 版本关系**：CLI `@awareness-sdk/claw` 和 Desktop `OCT-Agent.dmg` 是**两套独立发布渠道**，版本号互不绑定。CLI 面向命令行用户，走 `npx`；Desktop 面向 GUI 用户，走 dmg/exe 下载 + 后端 `latest-version` 端点。两边都要维护 CHANGELOG，都要在主仓 `docs/prd/deployment-log.md` 留痕。
