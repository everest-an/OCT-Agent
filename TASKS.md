...
- [ ] **记忆口径回答漂移修复**：用户询问“memory 是什么”时，agent 仍复述旧版 file-based 记忆叙事（daily notes + MEMORY.md），与 Awareness 双层记忆产品定义不一致；需统一运行时 AGENTS 提示词与聊天链路口径，避免继续输出过时说明（P2，2026-04-11 记录）
...
- [ ] **Gateway 一键修复（自动诊断 + 自愈）**：当配置热重载后 Gateway 自动重启未就绪（如 `RPC probe failed`/`1006`/`1008`）时，前端提供“一键修复”；主进程按顺序执行 health probe → pairing 修复 → gateway restart → Windows 服务兜底（install/start）→ 关键配置回滚提示，并输出可复制诊断摘要（P2，2026-04-13 记录）
