...
1. 复用优先，不重复造轮子：若能力已在 Awareness SDK 中存在，必须走依赖复用，不允许复制实现。
2. 超级傻瓜化：用户可见失败必须是非技术术语文案，并提供一键修复动作，不能把排错责任转嫁给用户。
3. 记忆是差异化：兼容性改造不能牺牲记忆功能可用性，记忆相关稳定性必须作为上线门禁指标。
...
3. 失败后提供一键修复动作。
4. 用户可导出诊断包。
5. 用户提示默认用人话，不直接展示底层异常堆栈或命令错误。
6. 禁止把“请打开终端执行命令”作为主路径，仅允许作为高级兜底信息。
...
1. SDK 复用率 >= 95%（新增能力优先复用 Awareness SDK）。
2. 零命令行闭环率 >= 90%（用户无需手工执行命令即可完成修复）。
3. 记忆可用率 >= 99.5%，跨会话召回命中率持续周环比提升。
