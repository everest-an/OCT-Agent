# AwarenessClaw 安装指南（macOS 测试版）

> 本文档说明如何在 macOS 上安装 AwarenessClaw 测试版 DMG。
...
4. **双击 `⚠️ 首次打开必读 First-Run Fix.command`**
   - 会弹出一个终端窗口显示进度
   - 显示 `✅ 完成！` 后按任意键关闭
5. 打开"**应用程序**"文件夹，双击 **AwarenessClaw** 图标正常启动
...
因为别的 app 都走了 **Apple Developer ID 签名 + 公证** 流程（每年 $99 会费 + 每次打包上传到 Apple 服务器公证）。AwarenessClaw 现在是内部测试版，还没走这个流程。**正式版发布时会走完整的签名+公证，届时这个文档就作废，用户下载直接双击打开**。
...
这些是 macOS 的权限弹窗，和"损坏"警告是两回事。选"允许"即可。AwarenessClaw 需要这些权限是因为：
...
**最后更新**: 2026-04-11
**适用版本**: AwarenessClaw ≥ 0.2.4（DMG 内置 fix-app.command 脚本）
